...
class PathModel(BaseModel):
    ...
    name: str = Field(..., description="The name of the directory or file.")
    is_dir: bool = Field(False, description="Specifies if this is a directory. Validated against `Path.is_dir`.")
    description: Optional[str] = Field(
        None, description="Description of the directory or file."
    )
    exists: Optional[NodeBoolModel] = Field(
        False, description="The directory/file must exist for validation to succeed."
    )
    resolve: Optional[str] = Field(
        None, description="Path to which this directory resolves (used for symlinks). Validated against `Path.resolve`."
    )
